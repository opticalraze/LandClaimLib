    ...
    public void onClaimPermissionCheck(ClaimPermissionCheckEvent event)
    {
        ...
        if (event.getRequiredPermission() == ClaimPermission.Edit)
        {
            if (claim.siegeData != null)
                event.setDenialReason(() -> LandClaimLib.instance.dataStore.getMessage(Messages.NoModifyDuringSiege));
            return;
        }
        ...
        if (event.getRequiredPermission() == ClaimPermission.Inventory)
        {
            ...
            LandClaimLib.instance.dataStore.tryExtendSiege(player, claim);

            if (claim.siegeData != null)
                event.setDenialReason(() -> LandClaimLib.instance.dataStore.getMessage(Messages.NoContainersSiege, claim.siegeData.attacker.getName()));
        ...
        if (broken != null)
        {
            ...
            if (!LandClaimLib.instance.config_siege_blocks.contains(broken))
                event.setDenialReason(() -> LandClaimLib.instance.dataStore.getMessage(Messages.NonSiegeMaterial));
            else if (player.getUniqueId().equals(claim.ownerID))
                event.setDenialReason(() -> LandClaimLib.instance.dataStore.getMessage(Messages.NoOwnerBuildUnderSiege));
            return;
        }

        // No building while under siege.
        if (claim.siegeData != null)
            event.setDenialReason(() -> LandClaimLib.instance.dataStore.getMessage(Messages.NoBuildUnderSiege, claim.siegeData.attacker.getName()));
