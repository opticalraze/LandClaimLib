...
public class PlayerData
{
    ...
    public boolean inPvpCombat()
    {
        ...
        if (elapsed > LandClaimLib.instance.config_pvp_combatTimeoutSeconds * 1000) //X seconds
        {
    ...
    {
        int remainingBlocks = this.getAccruedClaimBlocks() + this.getBonusClaimBlocks() + LandClaimLib.instance.dataStore.getGroupBonusBlocks(this.playerID);
    ...
    private void loadDataFromSecondaryStorage()
    {
        ...
        PlayerData storageData = LandClaimLib.instance.dataStore.getPlayerDataFromStorage(this.playerID);
        ...
        if (this.accruedClaimBlocks == null)
        {
            if (storageData.accruedClaimBlocks != null)
            {
                ...
                if (LandClaimLib.instance.config_advanced_fixNegativeClaimblockAmounts && (this.accruedClaimBlocks < LandClaimLib.instance.config_claims_initialBlocks))
                {
                    this.accruedClaimBlocks = LandClaimLib.instance.config_claims_initialBlocks;
                }
    ...
    public Vector<Claim> getClaims()
    {
        if (this.claims == null)
        {
            ...
            DataStore dataStore = LandClaimLib.instance.dataStore;
            ...
            int totalBlocks = this.accruedClaimBlocks + this.getBonusClaimBlocks() + LandClaimLib.instance.dataStore.getGroupBonusBlocks(this.playerID);
            if (LandClaimLib.instance.config_advanced_fixNegativeClaimblockAmounts && totalBlocks < totalClaimsArea)
            {
                OfflinePlayer player = LandClaimLib.instance.getServer().getOfflinePlayer(this.playerID);
                LandClaimLib.AddLogEntry(player.getName() + " has more claimed land than blocks available.  Adding blocks to fix.", CustomLogEntryTypes.Debug, true);
                LandClaimLib.AddLogEntry(player.getName() + " Accrued blocks: " + this.getAccruedClaimBlocks() + " Bonus blocks: " + this.getBonusClaimBlocks(), CustomLogEntryTypes.Debug, true);
                LandClaimLib.AddLogEntry("Total blocks: " + totalBlocks + " Total claimed area: " + totalClaimsArea, CustomLogEntryTypes.Debug, true);
                for (Claim claim : this.claims)
                {
                    if (!claim.inDataStore) continue;
                    LandClaimLib.AddLogEntry(
                            LandClaimLib.getfriendlyLocationString(claim.getLesserBoundaryCorner()) + " // "
                                    + LandClaimLib.getfriendlyLocationString(claim.getGreaterBoundaryCorner()) + " = "
                                    + claim.getArea()
                ...
                LandClaimLib.AddLogEntry("New accrued blocks: " + this.accruedClaimBlocks, CustomLogEntryTypes.Debug, true);

                //Recalculate total blocks (accrued + bonus + permission group bonus)
                totalBlocks = this.accruedClaimBlocks + this.getBonusClaimBlocks() + LandClaimLib.instance.dataStore.getGroupBonusBlocks(this.playerID);
                LandClaimLib.AddLogEntry("New total blocks: " + totalBlocks, CustomLogEntryTypes.Debug, true);
                ...
                if (totalBlocks < totalClaimsArea)
                {
                    ...
                    LandClaimLib.AddLogEntry("Accrued blocks weren't enough. Adding " + bonusBlocksToAdd + " bonus blocks.", CustomLogEntryTypes.Debug, true);
                }
                LandClaimLib.AddLogEntry(player.getName() + " Accrued blocks: " + this.getAccruedClaimBlocks() + " Bonus blocks: " + this.getBonusClaimBlocks() + " Group Bonus Blocks: " + LandClaimLib.instance.dataStore.getGroupBonusBlocks(this.playerID), CustomLogEntryTypes.Debug, true);
                //Recalculate total blocks (accrued + bonus + permission group bonus)
                totalBlocks = this.accruedClaimBlocks + this.getBonusClaimBlocks() + LandClaimLib.instance.dataStore.getGroupBonusBlocks(this.playerID);
                LandClaimLib.AddLogEntry("Total blocks: " + totalBlocks + " Total claimed area: " + totalClaimsArea, CustomLogEntryTypes.Debug, true);
                LandClaimLib.AddLogEntry("Remaining claim blocks to use: " + this.getRemainingClaimBlocks() + " (should be 0)", CustomLogEntryTypes.Debug, true);
            }
        }
    ...
    public int getAccruedClaimBlocksLimit()
    {
        if (this.AccruedClaimBlocksLimit < 0)
            return LandClaimLib.instance.config_claims_maxAccruedBlocks_default;
