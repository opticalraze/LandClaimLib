...
class PlayerRescueTask implements Runnable
{
    ...
    public void run()
    {
        ...
        {
            LandClaimLib.sendMessage(player, TextMode.Err, Messages.RescueAbortedMoved);
            return;
        }
        ...
        {
            this.destination = LandClaimLib.instance.ejectPlayer(this.player);
        }
        else
        {
        ...
        LandClaimLib.AddLogEntry("Rescued trapped player " + player.getName() + " from " + LandClaimLib.getfriendlyLocationString(this.location) + " to " + LandClaimLib.getfriendlyLocationString(this.destination) + ".");
    }
}
