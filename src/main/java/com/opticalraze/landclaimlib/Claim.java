...
import com.opticalraze.landclaimlib.Events.ClaimPermissionCheckEvent;
...
public class Claim
{
    ...
    public void removeSurfaceFluids(Claim exclusionClaim)
    {
        ...
        if (!LandClaimLib.instance.creativeRulesApply(this.lesserBoundaryCorner)) return;
        ...
        if (lesser.getWorld().getEnvironment() == Environment.NORMAL)
            seaLevel = LandClaimLib.instance.getSeaLevel(lesser.getWorld());
            ...
            @Nullable Event event)
    {
        ...
        if (uuid.equals(this.getOwnerID())
                || LandClaimLib.instance.dataStore.getPlayerData(uuid).ignoreClaims
        ...
        if (permission == ClaimPermission.Build)
        {
            ...
            PlayerData playerData = LandClaimLib.instance.dataStore.getPlayerData(uuid);
            if (playerData.inPvpCombat())
            {
                return () -> LandClaimLib.instance.dataStore.getMessage(Messages.NoBuildPvP);
            }
        ...
        return () ->
        {
            String reason = LandClaimLib.instance.dataStore.getMessage(permission.getDenialMessage(), this.getOwnerName());
            if (hasBypassPermission(player, permission))
                reason += "  " + LandClaimLib.instance.dataStore.getMessage(Messages.IgnoreClaimsAdvertisement);
            return reason;
        };
    }
    ...
    public String getOwnerName()
    {
        ...
        if (this.ownerID == null)
            return LandClaimLib.instance.dataStore.getMessage(Messages.OwnerNameForAdminClaims);

        return LandClaimLib.lookupPlayerName(this.ownerID);
    }
    ...
    public String allowMoreEntities(boolean remove)
    {
        ...
        if (!LandClaimLib.instance.creativeRulesApply(this.getLesserBoundaryCorner())) return null;
        ...
        if (maxEntities == 0) return LandClaimLib.instance.dataStore.getMessage(Messages.ClaimTooSmallForEntities);
        ...
        if (totalEntities >= maxEntities)
            return LandClaimLib.instance.dataStore.getMessage(Messages.TooManyEntitiesInClaim);
    ...
    public String allowMoreActiveBlocks()
    {
        ...
        if (maxActives == 0)
            return LandClaimLib.instance.dataStore.getMessage(Messages.ClaimTooSmallForActiveBlocks);
        ...
        if (totalActives >= maxActives)
            return LandClaimLib.instance.dataStore.getMessage(Messages.TooManyActiveBlocksInClaim);
    ...
    long getPlayerInvestmentScore()
    {
        ...
        boolean creativeMode = LandClaimLib.instance.creativeRulesApply(lesserBoundaryCorner);
        ...
        for (int x = this.lesserBoundaryCorner.getBlockX(); x <= this.greaterBoundaryCorner.getBlockX(); x++)
        {
            for (int z = this.lesserBoundaryCorner.getBlockZ(); z <= this.greaterBoundaryCorner.getBlockZ(); z++)
            {
                ...
                for (; y < LandClaimLib.instance.getSeaLevel(this.lesserBoundaryCorner.getWorld()) - 5; y++)
                {
