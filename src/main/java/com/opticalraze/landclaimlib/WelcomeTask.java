...
public class WelcomeTask implements Runnable
{
    ...
    public void run()
    {
        ...
        LandClaimLib.sendMessage(player, TextMode.Instr, Messages.AvoidGriefClaimLand);
        ...
        if (LandClaimLib.instance.config_claims_supplyPlayerManual)
        {
            ...
            String editToolName = LandClaimLib.instance.config_claims_modificationTool.name().replace('_', ' ').toLowerCase();
            String infoToolName = LandClaimLib.instance.config_claims_investigationTool.name().replace('_', ' ').toLowerCase();
