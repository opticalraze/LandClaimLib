...
class FindUnusedClaimsTask implements Runnable
{
    ...
    public void run()
    {
        ...
        LandClaimLib.instance.getServer().getScheduler().runTaskAsynchronously(LandClaimLib.instance, new CleanupUnusedClaimPreTask(claimOwnerIterator.next()));
    }

    public void refreshUUIDs()
    {
        // Fetch owner UUIDs from list of claims
        claimOwnerUUIDs = LandClaimLib.instance.dataStore.claims.stream().map(claim -> claim.ownerID)
        ...
        LandClaimLib.AddLogEntry("The following UUIDs own a claim and will be checked for inactivity in the following order:", CustomLogEntryTypes.Debug, true);

        for (UUID uuid : claimOwnerUUIDs)
            LandClaimLib.AddLogEntry(uuid.toString(), CustomLogEntryTypes.Debug, true);
