...
class EntityCleanupTask implements Runnable
{
    ...
    public void run()
    {
        ...
        for (World world : LandClaimLib.instance.getServer().getWorlds())
        {
        ...
        for (World world : worlds)
        {
            ...
            for (; j < entities.size() && j < k; j++)
            {
                ...
                {
                    Claim claim = LandClaimLib.instance.dataStore.getClaimAt(entity.getLocation(), false, cachedClaim);
                    if (claim != null)
                    {
                        cachedClaim = claim;
                    }
                ...
                if (remove)
                {
                    LandClaimLib.AddLogEntry("Removing entity " + entity.getType().name() + " @ " + entity.getLocation(), CustomLogEntryTypes.Debug, true);
                    entity.remove();
                }
            }
        }
        ...
        List<Claim> claims = LandClaimLib.instance.dataStore.claims;
        ...
        LandClaimLib.instance.getServer().getScheduler().scheduleSyncDelayedTask(LandClaimLib.instance, task, 20L * 60 * 1);
    }
}
