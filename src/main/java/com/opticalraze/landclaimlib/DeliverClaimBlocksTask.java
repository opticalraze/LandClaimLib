...
import com.opticalraze.landclaimlib.Events.AccrueClaimBlocksEvent;
...
class DeliverClaimBlocksTask implements Runnable
{
    ...
    public DeliverClaimBlocksTask(Player player, LandClaimLib instance)
    {
        this.player = player;
    ...
    public void run()
    {
        ...
        try
        {
            ...
            if (isIdle)
            {
                ...
                {
                    LandClaimLib.AddLogEntry(player.getName() + " wasn't active enough to accrue claim blocks this round.", CustomLogEntryTypes.Debug, true);
            ...
            {
                LandClaimLib.AddLogEntry(player.getName() + " claim block delivery was canceled by another plugin.", CustomLogEntryTypes.Debug, true);
            ...
            LandClaimLib.AddLogEntry("Delivering " + event.getBlocksToAccrue() + " blocks to " + player.getName(), CustomLogEntryTypes.Debug, true);
        ...
        {
            LandClaimLib.AddLogEntry("Problem delivering claim blocks to player " + player.getName() + ":");
            e.printStackTrace();
        }
    }
}
