...
class UUIDFetcher
{
    ...
    public void call() throws Exception
    {
        ...
        LandClaimLib.AddLogEntry("UUID conversion process started.  Please be patient - this may take a while.");

        LandClaimLib.AddLogEntry("Mining your local world data to save calls to Mojang...");
        OfflinePlayer[] players = LandClaimLib.instance.getServer().getOfflinePlayers();
        ...
        LandClaimLib.AddLogEntry("Checking local server data to get correct casing for player names...");
        for (int i = 0; i < names.size(); i++)
        {
            ...
            {
                LandClaimLib.AddLogEntry(name + " --> " + correctCasingName);
                names.set(i, correctCasingName);
            }
        }

        //look for local uuid's first
        LandClaimLib.AddLogEntry("Checking local server data for UUIDs already seen...");
        for (int i = 0; i < names.size(); i++)
        {
            String name = names.get(i);
            ...
            if (uuid != null)
            {
                LandClaimLib.AddLogEntry(name + " --> " + uuid.toString());
                names.remove(i--);
            }
        }
        ...
        if (LandClaimLib.instance.getServer().getOnlineMode())
        {
            ...
            names.removeIf(name ->
            {
                ...
                LandClaimLib.AddLogEntry(String.format("Cannot convert invalid name: %s", name));
                return true;
            });

            LandClaimLib.AddLogEntry("Calling Mojang to get UUIDs for remaining unresolved players (this is the slowest step)...");
                ...
                do
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        if (e.getMessage().contains("429"))
                        {
                            ...
                            {
                                LandClaimLib.AddLogEntry("Batch size " + PROFILES_PER_REQUEST + " seems too large.  Looking for a workable batch size...");
                            ...
                            {
                                LandClaimLib.AddLogEntry("Mojang says we're sending requests too fast.  Will retry every 30 seconds until we succeed...");
                                Thread.sleep(30000);
                            }
                        }
                        else
        ...
        {
            LandClaimLib.AddLogEntry("Generating offline mode UUIDs for remaining unresolved players...");
