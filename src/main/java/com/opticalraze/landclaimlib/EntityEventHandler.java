...
import com.opticalraze.landclaimlib.Events.PreventPvPEvent;
import com.opticalraze.landclaimlib.Events.ProtectDeathDropsEvent;
...
public class EntityEventHandler implements Listener
{
    ...
    private final LandClaimLib instance;
    private final NamespacedKey luredByPlayer;

    public EntityEventHandler(DataStore dataStore, LandClaimLib plugin)
    {
    ...
    public void onEntityFormBlock(EntityBlockFormEvent event)
    {
        ...
        if (entity.getType() == EntityType.PLAYER)
        {
            ...
            String noBuildReason = LandClaimLib.instance.allowBuild(player, event.getBlock().getLocation(), event.getNewState().getType());
    ...
    {
        if (!LandClaimLib.instance.config_endermenMoveBlocks && event.getEntityType() == EntityType.ENDERMAN)
        {
            event.setCancelled(true);
        }
        else if (!LandClaimLib.instance.config_silverfishBreakBlocks && event.getEntityType() == EntityType.SILVERFISH)
        {
            event.setCancelled(true);
        }
        else if (!LandClaimLib.instance.config_rabbitsEatCrops && event.getEntityType() == EntityType.RABBIT)
        {
            event.setCancelled(true);
        }
        else if (!LandClaimLib.instance.config_claims_ravagersBreakBlocks && event.getEntityType() == EntityType.RAVAGER)
        {
            event.setCancelled(true);
        }
        // All other handling depends on claims being enabled.
        else if (LandClaimLib.instance.config_claims_worldModes.get(event.getBlock().getWorld()) == ClaimsMode.Disabled)
        {
            return;
        }
        ...
        else if (event.getEntityType() == EntityType.WITHER)
        {
            ...
            if (claim == null || !claim.areExplosivesAllowed || !LandClaimLib.instance.config_blockClaimExplosions)
            {
                event.setCancelled(true);
            }
        }
        ...
        else if (event.getTo() == Material.DIRT && event.getBlock().getType() == Material.FARMLAND)
        {
            ...
            else
            {
                ...
                if (LandClaimLib.instance.allowBreak(player, block, block.getLocation()) != null)
                {
                    event.setCancelled(true);
                }
            }
        ...
        else if (event.getEntity() instanceof Vehicle && !event.getEntity().getPassengers().isEmpty())
        {
            ...
            {
                Block block = event.getBlock();
                if (LandClaimLib.instance.allowBreak((Player) driver, block, block.getLocation()) != null)
                {
                    event.setCancelled(true);
                }
            }
        ...
        else if (event.getEntityType() == EntityType.FALLING_BLOCK)
        {
            ...
            {
                entity.setMetadata("GP_FALLINGBLOCK", new FixedMetadataValue(LandClaimLib.instance, block.getLocation()));
            }
            ...
            else
            {
                ...
                if (originalLocation.getBlockX() != newLocation.getBlockX() || originalLocation.getBlockZ() != newLocation.getBlockZ())
                {
                    ...
                    if (LandClaimLib.instance.config_claims_worldModes.get(newLocation.getWorld()) == ClaimsMode.Creative)
                    {
                        event.setCancelled(true);
                        entity.remove();
                        return;
    ...
    private void handleProjectileChangeBlock(EntityChangeBlockEvent event, Projectile projectile)
    {
        ...
        if (claim == null)
        {
            ...
            {
                if (!LandClaimLib.instance.config_fireDestroys || !LandClaimLib.instance.config_fireSpreads)
                    event.setCancelled(true);
                return;
            }
        ...
        if (shooter instanceof Player)
        {
            ...
            if (denial != null)
            {
                ...
                LandClaimLib.sendMessage((Player) shooter, TextMode.Err, denial.get());
                event.setCancelled(true);
            }
    ...
    private boolean isBlockSourceInClaim(ProjectileSource projectileSource, Claim claim)
    {
        return projectileSource instanceof BlockProjectileSource &&
                LandClaimLib.instance.dataStore.getClaimAt(((BlockProjectileSource) projectileSource).getBlock().getLocation(), false, claim) == claim;
    }
    ...
    {
        if (!LandClaimLib.instance.config_zombiesBreakDoors) event.setCancelled(true);
    }
    ...
    public void onEntityInteract(EntityInteractEvent event)
    {
        ...
        {
            if (!LandClaimLib.instance.config_creaturesTrampleCrops)
            {
                event.setCancelled(true);
            }
            else
    ...
    void handleExplosion(Location location, Entity entity, List<Block> blocks)
    {
        ...
        if (!LandClaimLib.instance.claimsEnabledForWorld(world)) return;
        ...
        boolean applySurfaceRules = world.getEnvironment() == Environment.NORMAL && ((isCreeper && LandClaimLib.instance.config_blockSurfaceCreeperExplosions) || (!isCreeper && LandClaimLib.instance.config_blockSurfaceOtherExplosions));

        //special rule for creative worlds: explosions don't destroy anything
        if (LandClaimLib.instance.creativeRulesApply(location))
        {
        ...
        for (Block block : blocks)
        {
            ...
            if (claim != null && claim.siegeData != null)
            {
                ...
                boolean breakable = LandClaimLib.instance.config_siege_blocks.contains(material);
            ...
            if (claim == null)
            {
                if (!applySurfaceRules || block.getLocation().getBlockY() < LandClaimLib.instance.getSeaLevel(world) - 7)
                {
                    explodedBlocks.add(block);
                }
            }
    ...
    public void onItemSpawn(ItemSpawnEvent event)
    {
        ...
        if (LandClaimLib.instance.creativeRulesApply(event.getLocation()))
        {
            event.setCancelled(true);
        }

        //if item is on watch list, apply protection
        ArrayList<PendingItemProtection> watchList = LandClaimLib.instance.pendingItemWatchList;
        ...
        for (int i = 0; i < watchList.size(); i++)
        {
            ...
            newItem.setMetadata("GP_ITEMOWNER", new FixedMetadataValue(LandClaimLib.instance, pendingProtection.owner));
    ...
    public void onExpBottle(ExpBottleEvent event)
    {
        ...
        if (LandClaimLib.instance.creativeRulesApply(event.getEntity().getLocation()))
        {
    ...
    public void onEntitySpawn(CreatureSpawnEvent event)
    {
        ...
        if (!LandClaimLib.instance.creativeRulesApply(event.getLocation())) return;
    ...
    public void onEntityDeath(EntityDeathEvent event)
    {
        ...
        if (!LandClaimLib.instance.claimsEnabledForWorld(entity.getWorld())) return;

        //special rule for creative worlds: killed entities don't drop items or experience orbs
        if (LandClaimLib.instance.creativeRulesApply(entity.getLocation()))
        {
        ...
        boolean isPvPWorld = LandClaimLib.instance.pvpRulesApply(world);
        if ((isPvPWorld && LandClaimLib.instance.config_lockDeathDropsInPvpWorlds) ||
                (!isPvPWorld && LandClaimLib.instance.config_lockDeathDropsInNonPvpWorlds))
        {
            ...
            if (!protectionEvent.isCancelled())
            {
                ...
                {
                    LandClaimLib.instance.pendingItemWatchList.add(
    ...
    public void onHangingBreak(HangingBreakEvent event)
    {
        ...
        if (!LandClaimLib.instance.claimsEnabledForWorld(event.getEntity().getWorld())) return;
        ...
        String noBuildReason = LandClaimLib.instance.allowBuild(playerRemover, event.getEntity().getLocation(), Material.AIR);
        if (noBuildReason != null)
        {
            event.setCancelled(true);
            LandClaimLib.sendMessage(playerRemover, TextMode.Err, noBuildReason);
        }
    }
    ...
    {
        //don't track in worlds where claims are not enabled
        if (!LandClaimLib.instance.claimsEnabledForWorld(event.getBlock().getWorld())) return;

        //FEATURE: similar to above, placing a painting requires build permission in the claim

        //if the player doesn't have permission, don't allow the placement
        String noBuildReason = LandClaimLib.instance.allowBuild(event.getPlayer(), event.getEntity().getLocation(), Material.PAINTING);
        if (noBuildReason != null)
        {
            event.setCancelled(true);
            LandClaimLib.sendMessage(event.getPlayer(), TextMode.Err, noBuildReason);
            return;
        }

        //otherwise, apply entity-count limitations for creative worlds
        else if (LandClaimLib.instance.creativeRulesApply(event.getEntity().getLocation()))
        {
            ...
            {
                LandClaimLib.sendMessage(event.getPlayer(), TextMode.Err, noEntitiesReason);
                event.setCancelled(true);
                return;
            }
        }
    ...
    private void handleEntityDamageEvent(EntityDamageEvent event, boolean sendErrorMessagesToPlayers)
    {
        ...
        if (event.getEntity() instanceof Horse && !LandClaimLib.instance.config_claims_protectHorses) return;
        if (event.getEntity() instanceof Donkey && !LandClaimLib.instance.config_claims_protectDonkeys) return;
        if (event.getEntity() instanceof Mule && !LandClaimLib.instance.config_claims_protectDonkeys) return;
        if (event.getEntity() instanceof Llama && !LandClaimLib.instance.config_claims_protectLlamas) return;
        ...
        if (event.getEntity() instanceof Tameable && !LandClaimLib.instance.pvpRulesApply(event.getEntity().getWorld()))
        {
        ...
        if (damageSource != null)
        {
            ...
            if (damageSource.getType() == EntityType.AREA_EFFECT_CLOUD && event.getEntityType() == EntityType.PLAYER && LandClaimLib.instance.pvpRulesApply(event.getEntity().getWorld()))
            {
                Player damaged = (Player) event.getEntity();
                PlayerData damagedData = LandClaimLib.instance.dataStore.getPlayerData(damaged.getUniqueId());

                //case 1: recently spawned
                if (LandClaimLib.instance.config_pvp_protectFreshSpawns && damagedData.pvpImmune)
                {
                    event.setCancelled(true);
                    return;
                }
                ...
                {
                    Claim damagedClaim = LandClaimLib.instance.dataStore.getClaimAt(damaged.getLocation(), false, damagedData.lastClaim);
                    if (damagedClaim != null)
                    {
                        damagedData.lastClaim = damagedClaim;
                        if (LandClaimLib.instance.claimIsPvPSafeZone(damagedClaim))
                        {
        ...
        if (firework != null && event.getEntityType() == EntityType.PLAYER && !LandClaimLib.instance.pvpRulesApply(attacker.getWorld()))
        {
        ...
        if (attacker != null && event.getEntityType() == EntityType.PLAYER && LandClaimLib.instance.pvpRulesApply(attacker.getWorld()))
        {
            ...
            if (attacker != defender)
            {
                ...
                if (LandClaimLib.instance.config_pvp_protectFreshSpawns)
                {
                    if (defenderData.pvpImmune)
                    {
                        event.setCancelled(true);
                        if (sendErrorMessagesToPlayers)
                            LandClaimLib.sendMessage(attacker, TextMode.Err, Messages.ThatPlayerPvPImmune);
                        return;
                    }
                    ...
                    if (attackerData.pvpImmune)
                    {
                        event.setCancelled(true);
                        if (sendErrorMessagesToPlayers)
                            LandClaimLib.sendMessage(attacker, TextMode.Err, Messages.CantFightWhileImmune);
                        return;
                    }
                }

                //FEATURE: prevent players from engaging in PvP combat inside land claims (when it's disabled)
                if (LandClaimLib.instance.config_pvp_noCombatInPlayerLandClaims || LandClaimLib.instance.config_pvp_noCombatInAdminLandClaims)
                {
                    ...
                    if (!attackerData.ignoreClaims)
                    {
                        if (attackerClaim != null && //ignore claims mode allows for pvp inside land claims
                                ...
                                LandClaimLib.instance.claimIsPvPSafeZone(attackerClaim))
                        {
                        ...
                        if (defenderClaim != null &&
                                ...
                                LandClaimLib.instance.claimIsPvPSafeZone(defenderClaim))
                        {
                            ...
                            if (!pvpEvent.isCancelled())
                            {
                                event.setCancelled(true);
                                if (sendErrorMessagesToPlayers)
                                    LandClaimLib.sendMessage(attacker, TextMode.Err, Messages.PlayerInPvPSafeZone);
                            }
                            return;
                        }
                    }
        ...
        if (event instanceof EntityDamageByEntityEvent)
        {
            ...
            if (event.getEntityType() == EntityType.PLAYER)
            {
                ...
                if (damager != null && damager instanceof Tameable)
                {
                    ...
                    if (pet.isTamed() && pet.getOwner() != null)
                    {
                        ...
                        PlayerData defenderData = LandClaimLib.instance.dataStore.getPlayerData(event.getEntity().getUniqueId());
                    ...
                    || subEvent.getEntityType() == EntityType.ENDER_CRYSTAL)
            {
                ...
                if (subEvent.getEntityType() == EntityType.VILLAGER && !LandClaimLib.instance.config_claims_protectCreatures)
                    return;
                ...
                if (claim != null)
                {
                    ...
                    if (failureReason != null)
                    {
                        event.setCancelled(true);
                        if (sendErrorMessagesToPlayers)
                            LandClaimLib.sendMessage(attacker, TextMode.Err, failureReason.get());
                        return;
                    }
                }
            }
            ...
            if (((subEvent.getEntity() instanceof Creature || subEvent.getEntity() instanceof WaterMob) && LandClaimLib.instance.config_claims_protectCreatures))
            {
                ...
                if (subEvent.getEntity() instanceof Tameable)
                {
                    ...
                    if (tameable.isTamed() && tameable.getOwner() != null)
                    {
                        ...
                        if (attacker != null)
                        {
                            ...
                            if (!LandClaimLib.instance.pvpRulesApply(subEvent.getEntity().getLocation().getWorld()) || (LandClaimLib.instance.config_pvp_protectPets && subEvent.getEntityType() != EntityType.WOLF))
                            {
                                OfflinePlayer owner = LandClaimLib.instance.getServer().getOfflinePlayer(ownerID);
                                String ownerName = owner.getName();
                                if (ownerName == null) ownerName = "someone";
                                String message = LandClaimLib.instance.dataStore.getMessage(Messages.NoDamageClaimedEntity, ownerName);
                                if (attacker.hasPermission("griefprevention.ignoreclaims"))
                                    message += "  " + LandClaimLib.instance.dataStore.getMessage(Messages.IgnoreClaimsAdvertisement);
                                if (sendErrorMessagesToPlayers)
                                    LandClaimLib.sendMessage(attacker, TextMode.Err, message);
                            ...
                            else if (tameable.getType() == EntityType.WOLF)
                            {
                                if (!tameable.getOwner().equals(attacker))
                                {
                                    ...
                                    event.setCancelled(true);
                                    String ownerName = LandClaimLib.instance.getServer().getOfflinePlayer(ownerID).getName();
                                    String message = LandClaimLib.instance.dataStore.getMessage(Messages.NoDamageClaimedEntity, ownerName);
                                    if (attacker.hasPermission("griefprevention.ignoreclaims"))
                                        message += "  " + LandClaimLib.instance.dataStore.getMessage(Messages.IgnoreClaimsAdvertisement);
                                    if (sendErrorMessagesToPlayers)
                ...
                if (claim != null)
                {
                    ...
                    else if (!(event.getEntity().getWorld().getPVP() && event.getEntity().getType() == EntityType.WOLF))
                    {
                        ...
                        if (sendErrorMessagesToPlayers)
                        {
                            ...
                            override = () ->
                            {
                                String message = LandClaimLib.instance.dataStore.getMessage(Messages.NoDamageClaimedEntity, claim.getOwnerName());
                        ...
                        if (noContainersReason != null)
                        {
                            ...
                            if (sendErrorMessagesToPlayers)
                            {
                                LandClaimLib.sendMessage(attacker, TextMode.Err, noContainersReason.get());
                            }
                            event.setCancelled(true);
                        }
    ...
    private boolean handleBlockExplosionDamage(EntityDamageEvent event)
    {
        ...
        Claim claim = LandClaimLib.instance.dataStore.getClaimAt(entity.getLocation(), false, null);
    ...
    public void onCrossbowFireWork(EntityShootBowEvent shootEvent)
    {
        ...
        {
            shootEvent.getProjectile().setMetadata("GP_FIREWORK", new FixedMetadataValue(LandClaimLib.instance, shootEvent.getEntity()));
        }
    }
    ...
    public void onEntityDamageMonitor(EntityDamageEvent event)
    {
        ...
        if (!LandClaimLib.instance.pvpRulesApply(defender.getWorld())) return;
    ...
    public void onVehicleDamage(VehicleDamageEvent event)
    {
        ...
        if (!LandClaimLib.instance.config_claims_preventTheft) return;

        //input validation
        if (event.getVehicle() == null) return;

        //don't track in worlds where claims are not enabled
        if (!LandClaimLib.instance.claimsEnabledForWorld(event.getVehicle().getWorld())) return;
        ...
        for (PotionEffect effect : effects)
        {
                        ...
                        if (claim != null)
                        {
                            ...
                            else
                            {
                                ...
                                if (noContainersReason != null)
                                {
                                    ...
                                    if (!messagedPlayer)
                                    {
                                        LandClaimLib.sendMessage(thrower, TextMode.Err, noContainersReason.get());
                                        messagedPlayer = true;
                                    }
                                }
                            }
            ...
            for (LivingEntity effected : event.getAffectedEntities())
            {
                ...
                if (effected.getType() != EntityType.PLAYER) continue;
                    ...
                    //FEATURE: prevent players from engaging in PvP combat inside land claims (when it's disabled)
                else if (LandClaimLib.instance.config_pvp_noCombatInPlayerLandClaims || LandClaimLib.instance.config_pvp_noCombatInAdminLandClaims)
                {
                    ...
                    if (attackerClaim != null && LandClaimLib.instance.claimIsPvPSafeZone(attackerClaim))
                    {
                        attackerData.lastClaim = attackerClaim;
                        ...
                        if (!pvpEvent.isCancelled())
                        {
                            ...
                            if (!messagedPlayer)
                            {
                                LandClaimLib.sendMessage(thrower, TextMode.Err, Messages.CantFightWhileImmune);
                                messagedPlayer = true;
                            }
                        }
                        continue;
                    ...
                    if (defenderClaim != null && LandClaimLib.instance.claimIsPvPSafeZone(defenderClaim))
                    {
                        defenderData.lastClaim = defenderClaim;
                        ...
                        if (!pvpEvent.isCancelled())
                        {
                            ...
                            if (!messagedPlayer)
                            {
                                LandClaimLib.sendMessage(thrower, TextMode.Err, Messages.PlayerInPvPSafeZone);
                                messagedPlayer = true;
                            }
                        }
                    }
